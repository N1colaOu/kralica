...
void GravityForce::compute(const std::vector<Body>& system, std::vector<Vector2d>& accs) const{
    ...
    {   
        ...
        for (size_t j = i + 1; j < n; j++)
        {
            ...
            const double rad_norm2 = rad_ij[0]*rad_ij[0] + rad_ij[1]*rad_ij[1];
            const double rad_soft = 1/std::sqrt(rad_norm2 + R_SOFT_GRAV*R_SOFT_GRAV);
...
void LennardJonesForce::compute(const std::vector<Body>& system, std::vector<Vector2d>& accs) const{
    ...
    {   
        ...
        for (size_t j = i + 1; j < n; j++)
        {
            ...
            const Vector2d rad_ij = (pos_j-pos_i);
            const double rad_norm2 = rad_ij[0]*rad_ij[0] + rad_ij[1]*rad_ij[1];
            if (rad_norm2 >= R_CUTOFF_LJ) continue;
            const double m_j = particle_j.get_mass();
            const double x = SIGMA_LJ * SIGMA_LJ / (rad_norm2 + R_SOFT_LJ * R_SOFT_LJ);
            double x3 = x * x * x;
            double x6 = x3 * x3;
